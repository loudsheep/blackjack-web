...
export function useBlackjack() {
  ...
  const handleServerEvent = (msg: any, gameId: string) => {
    switch (msg.event) {
      ...
      case "Kicked":
      case "Error":
        const errorMessage = !msg.data ? "" : (typeof msg.data === 'string' ? msg.data : (msg.data.msg || JSON.stringify(msg.data)));
        console.error("Server Error/Kicked:", errorMessage, "Event:", msg.event);

        // Handle "Kicked" message
        if (msg.event === "Kicked" || (errorMessage && errorMessage.toLowerCase().includes("kick"))) {
          console.warn("Kick detected. Forcing disconnect and state clear.");
          setConnectionError({
            title: "You Have Been Kicked",
            msg: "An administrator has removed you from the game room."
          });

          // Force state clear immediately to update UI
          setGameState(null);
          setIsConnected(false);
          setMyPlayerId(null);
          setIsAdmin(false);
          if (gameId) {
            console.log(`Removing stored auth for ${gameId} due to kick`);
            localStorage.removeItem(`blackjack_auth_${gameId}`);
          }
